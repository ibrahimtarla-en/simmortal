...
export function DataTable({ columns, data }: DataTableProps<DetailedOrder, DetailedOrder>) {
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV<DetailedOrder>(dataToExport, `orders-export-${format(new Date(), 'yyyy-MM-dd')}`, [
      ...
      { key: 'memorialId', header: 'Memorial ID' },
      ...
      { key: 'stripePaymentId', header: 'Stripe Payment ID' },
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  key={row.id}
                  onClick={() => {
                    router.push(`orders/${(row.original as DetailedOrder).id}`);
                  }}
